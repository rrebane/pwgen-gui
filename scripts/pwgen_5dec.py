...
def asciiToKeyboardenc(inAscii):
    out = []
    asciiDict = dict([(a,k) for k,a in keyboardDict.items()])
    for c in inAscii:
...
def bruteForce(hash, salt=0, digitsOnly=False, charsOnly=True, minLen=3, maxLen=8):
    ...
    if digitsOnly:
        keyboardDict = dict(list(zip(list(keyboardDict.keys())[0:9],list(keyboardDict.values())[0:9])))
    elif charsOnly:
        keyboardDict = dict(list(zip(list(keyboardDict.keys())[10:36],list(keyboardDict.values())[10:36])))
